...
$app->get("/order/:idorder/pagseguro", function ($idorder) {
    ...
    $page->setTpl("payment-pagseguro", [
        ...
        'products' => $cart->getProducts(),
        'phone' => [
            'areCode' => substr($order->getnrphone(), 0, 2),
...
$app->post('/profile', function () {
    ...
    if (!isset($_POST['desemail']) || $_POST['desemail'] == '') {
        User::setError('Digite seu email');
        header("Location: /profile");
        exit;
    }
    ...
    if ($_POST['desperson'] !== $user->getdesemail()) {
        if (User::checkLoginExist($_POST['desperson'])) {
...
$app->get('/boleto/:idorder', function ($idorder) {
    ...
    // NÃO ALTERAR!
    $path = $_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . "resource" . DIRECTORY_SEPARATOR .
...
$app->post('/profile/change-password', function () {
    ...
    if ($_POST['current_pass']  === $_POST['new_pass']) {
        User::setError("Nova senha não pode ser igual a sua última senha");
        header("Location: /profile/change-password");
        exit;
    }
